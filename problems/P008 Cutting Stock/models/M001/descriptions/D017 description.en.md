...
To eliminate identical alternative configurations and break symmetry, the available rods must be utilized in a strict sequential order. This means that a given rod can only be selected for cutting if all preceding rods in the sequence are also chosen.

[//]: # (Generated using nemotron-3-nano:latest from D010 description.en.md and model.mzn; major manual amendments applied; added generated by gemini 3.5 flash statement with symmetry-breaking)
