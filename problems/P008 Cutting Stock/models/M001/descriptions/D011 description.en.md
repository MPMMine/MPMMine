...
Each customer order requests several distinct **item types**. For item type *i*, the required segment has a specific **dimension** denoted by **w_i**, and the total number of segments needed for this type is **q_i**.
...
Additionally, to eliminate symmetrical and redundant configurations, the available master rolls are indexed sequentially, and a master roll can only be utilized if the immediately preceding roll in the sequence is also utilized.
...
[//]: # (Generated using deepseek-r1:latest from D010 description.en.md and model.mzn; minor manual amendments applied; added generated by gemini 3.5 flash statement with symmetry-breaking)
