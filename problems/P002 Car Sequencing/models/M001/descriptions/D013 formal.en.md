...
Let $\text{need}\_o = \sum\_{c} q\_c\\,\text{opt}[c,o]$ denote the total number of cars that will require option $o$.
...
1. **Class counts**  
   $\forall c\in\mathcal C:\\;\\;\sum\_{s\in\mathcal S}\mathbf{1}[s\_s=c] = q\_c$.

2. **Block capacity**  
   $\forall o\in\mathcal O,\\;\forall s\in\\{1,\dots ,n\_{\text{cars}}-b\_o+1\\}:\\;
   \sum_{j=s}^{s+b_o-1} f_{o,j}\;\le\; m_o$.

3. **Feature consistency**  
   $\forall o\in\mathcal O,\\;\forall s\in\mathcal S:\\;
   f\_{o,s} = \text{opt}[s\_s,o]$.

4. **Global requirement per option**  
   For each option $o$ and each integer $k$ with $1\le k\le \text{need}\_o$,
   $\sum_\{s=1}^{\\,n\_{\text{cars}}-k\,b\_o}\\! f\_{o,s}
   \\;\ge\\; \text{need}\_o - k\\,m\_o$.
