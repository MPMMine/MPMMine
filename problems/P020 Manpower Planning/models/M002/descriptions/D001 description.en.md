...
There is a natural wastage of labor. A fairly large number of workers leave during their first year. After this, the rate is much smaller. Taking this into account, the wastage rates can be found in the table `retention_existing` and `retention_new`.
...
It is possible to retrain up to `max_retrain_unskilled` unskilled workers per year to make them semi-skilled. The retraining of semi-skilled workers to make them skilled is limited to no more than `promotion_limit` fraction of the skilled labor force at the time as some training is done on the job. Retraining costs are gathered in the `retrain_cost` table.
...
An employee on short-time working meets the production requirements of `short_time_efficiency` of a full-time employee. The company’s declared objective is to **minimise cost**. How should they operate in order to do this?
