...
2. The total travel time of a van cannot exceed the allowed limit:  
   \(\sum_{i,j} D[i,j]·x[i,j,k] ≤ T\) for each van `k`.  
3. Every city except the depot must be served by exactly one van: \(\sum_{k} y[i,k] = 1\) for all `i ≠ 1`.  
4. The depot is visited by each van that is used: \(\sum_{k} y[1,k] ≥ \sum_{k} z[k]\).  
5. Flow conservation: if a van enters a city it must leave it, and vice‑versa:  
   \(\sum_{i≠j} x[i,j,k] = y[j,k]\) and \(\sum_{i≠j} x[j,i,k] = y[j,k]\) for all `j, k`.  
6. Sub‑circuit (subtour) elimination is enforced through a successor array that creates a single closed route for each used van.  
7. Symmetry breaking: for each `k = 1…nV‑1`, the number of visited cities by van `k` is at least that of van `k+1`.  
