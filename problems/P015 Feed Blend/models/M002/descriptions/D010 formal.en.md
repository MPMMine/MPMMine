...
- **Grain share** – The set $\mathcal{G}\subseteq\mathcal{I}$ denotes all grain ingredients.  
  ...
  $$
  \sum_{i\in\mathcal{G}}\text{amount}[i,t] \ge 0.2 \text{total_weight}.
  $$

- **Inventory dynamics** – For each ingredient a warehouse with capacity $\text{storage_cap}$ stores the
  stock $\text{stock}[i,t]$ at the end of period $t$.  
  The initial stock is $\text{initial_stock}[i]$.  
...
- **Objective** – Minimize the sum of purchasing costs and holding costs over the horizon:
  ...
  \bigl(\text{buy}[i,t]\cdot\text{cost}[i,t]+\text{stock}[i,t]\cdot\text{holding_cost}\bigr).
  $$
