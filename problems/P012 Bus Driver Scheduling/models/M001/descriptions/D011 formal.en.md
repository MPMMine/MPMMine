...
1. $\text{tot}_\text{shifts} = \sum_{s\in S} x_s$ (definition of the total).
2. For every work item $w\in W$,  
   $$
   \sum_{\substack{s\in S: w\in s}} x_s = 1,
   $$
   ensuring that $w$ is covered exactly once.
3. $\text{tot}_\text{shifts} \ge m$, where $m$ is the pre‑specified minimum number of shifts required.
