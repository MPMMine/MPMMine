...
of products of corresponding elements separated by $m$ positions, i.e., $\sum\limits_{p=1}^{\lambda-m} X_p * X_{p+m}$.
The objective is to minimize the sum of squares of these self-correlations, denoted
as $Y=\sum\limits_{m=1}^{\lambda-1} R_m^2$, where each $R_m$ contributes to the overall measure of self-similarity.
