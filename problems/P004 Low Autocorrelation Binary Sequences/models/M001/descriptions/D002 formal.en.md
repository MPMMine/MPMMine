...
i.e., $\sum\limits_{j=1}^{\lambda-k} \Gamma_j * \Gamma_{j+k}$. The objective is to minimize the sum of squares of these
inter-element correlations, represented as $\Upsilon = \sum\limits_{k=1}^{\lambda-1} \Phi_k^2$.
