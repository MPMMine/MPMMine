...
namely $R_m = \sum\limits_{j=1}^{N-m} X_j * X_{j+m}$. Our objective is to reduce the sum of the squares of these
autocorrelation values. Thus, we want to minimize $E = \sum\limits_{m=1}^{N-1} R_m^2$.
