...
namely $R_l = \sum\limits_{j=1}^{N-l} X_j * X_{j+l}$. We seek to minimize the total squared correlation value,
represented by the expression $T = \sum\limits_{l=1}^{N-1} R_l^2$. Essentially, we aim to create a sequence where
