...
To accelerate the optimization process, structural properties are enforced via symmetry-breaking and implied constraints. Variants with identical demand levels are rendered interchangeable; to eliminate redundant symmetric search states, their slot allocations across all templates are organized in a strict lexicographical sequence. Analogously, for variants with strictly ascending order of demand, a pseudo-symmetry constraint mandates that their cumulative printed yields must follow a non-decreasing order. Additionally, valid bounds are imposed on the over-production surplus, dictating that neither the surplus of an individual variant nor the accumulated surplus of the first few variants can exceed the total global surplus. Finally, when evaluating configurations involving exactly two or three templates, fractional constraints restrict the relative run lengths of the templates to preclude rotational symmetries.
