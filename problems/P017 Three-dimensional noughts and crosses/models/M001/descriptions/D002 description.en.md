...
You are supplied with $n^3 - total\\_black\\_balls$ light-coloured markers (noughts) and $total\\_black\\_balls$ dark-coloured markers (crosses), where $total\\_black\\_balls$ is a prescribed constant. Place exactly one marker in every cell so that the number of rows in which all $n$ entries share the same colour is minimised.
